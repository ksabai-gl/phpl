...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/webhookstudio/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'WebhookStudio-Item-' + (index + 1),
...
export default function WebhookStudioItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WebhookStudio', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WebhookStudio', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WebhookStudio-Item">
      <header>
        <h1>WebhookStudio / Item</h1>
