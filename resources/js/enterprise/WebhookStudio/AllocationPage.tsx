...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/webhookstudio/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'WebhookStudio-Allocation-' + (index + 1),
...
export default function WebhookStudioAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WebhookStudio', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WebhookStudio', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WebhookStudio-Allocation">
      <header>
        <h1>WebhookStudio / Allocation</h1>
