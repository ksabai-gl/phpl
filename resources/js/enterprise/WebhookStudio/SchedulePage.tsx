...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/webhookstudio/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'WebhookStudio-Schedule-' + (index + 1),
...
export default function WebhookStudioSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WebhookStudio', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WebhookStudio', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WebhookStudio-Schedule">
      <header>
        <h1>WebhookStudio / Schedule</h1>
