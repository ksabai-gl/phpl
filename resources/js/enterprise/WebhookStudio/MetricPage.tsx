...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/webhookstudio/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'WebhookStudio-Metric-' + (index + 1),
...
export default function WebhookStudioMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WebhookStudio', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WebhookStudio', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WebhookStudio-Metric">
      <header>
        <h1>WebhookStudio / Metric</h1>
