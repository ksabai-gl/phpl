...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/webhookstudio/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'WebhookStudio-Record-' + (index + 1),
...
export default function WebhookStudioRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WebhookStudio', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WebhookStudio', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WebhookStudio-Record">
      <header>
        <h1>WebhookStudio / Record</h1>
