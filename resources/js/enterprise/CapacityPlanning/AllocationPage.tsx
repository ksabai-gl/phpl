...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/capacityplanning/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'CapacityPlanning-Allocation-' + (index + 1),
...
export default function CapacityPlanningAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CapacityPlanning', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CapacityPlanning', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CapacityPlanning-Allocation">
      <header>
        <h1>CapacityPlanning / Allocation</h1>
