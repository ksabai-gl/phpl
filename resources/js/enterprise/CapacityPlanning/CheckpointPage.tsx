...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/capacityplanning/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'CapacityPlanning-Checkpoint-' + (index + 1),
...
export default function CapacityPlanningCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CapacityPlanning', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CapacityPlanning', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CapacityPlanning-Checkpoint">
      <header>
        <h1>CapacityPlanning / Checkpoint</h1>
