...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/capacityplanning/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'CapacityPlanning-Metric-' + (index + 1),
...
export default function CapacityPlanningMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CapacityPlanning', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CapacityPlanning', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CapacityPlanning-Metric">
      <header>
        <h1>CapacityPlanning / Metric</h1>
