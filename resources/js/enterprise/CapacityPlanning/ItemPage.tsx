...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/capacityplanning/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'CapacityPlanning-Item-' + (index + 1),
...
export default function CapacityPlanningItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CapacityPlanning', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CapacityPlanning', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CapacityPlanning-Item">
      <header>
        <h1>CapacityPlanning / Item</h1>
