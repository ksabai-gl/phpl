...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/capacityplanning/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'CapacityPlanning-Record-' + (index + 1),
...
export default function CapacityPlanningRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CapacityPlanning', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CapacityPlanning', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CapacityPlanning-Record">
      <header>
        <h1>CapacityPlanning / Record</h1>
