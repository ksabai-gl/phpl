...
async function fetchBatchRows(): Promise<BatchRow[]> {
  const response = await fetch('/api/v1/enterprise/loyalty/batch');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Batch ' + (index + 1),
      code: 'Loyalty-Batch-' + (index + 1),
...
export default function LoyaltyBatchPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['Loyalty', 'Batch', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['Loyalty', 'Batch'] });
    },
  });
  ...
  return (
    <section className="enterprise-module Loyalty-Batch">
      <header>
        <h1>Loyalty / Batch</h1>
