...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/projectportfolio/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'ProjectPortfolio-Item-' + (index + 1),
...
export default function ProjectPortfolioItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ProjectPortfolio', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ProjectPortfolio', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ProjectPortfolio-Item">
      <header>
        <h1>ProjectPortfolio / Item</h1>
