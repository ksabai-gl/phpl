...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/projectportfolio/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'ProjectPortfolio-Record-' + (index + 1),
...
export default function ProjectPortfolioRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ProjectPortfolio', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ProjectPortfolio', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ProjectPortfolio-Record">
      <header>
        <h1>ProjectPortfolio / Record</h1>
