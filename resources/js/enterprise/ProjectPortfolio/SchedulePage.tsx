...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/projectportfolio/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'ProjectPortfolio-Schedule-' + (index + 1),
...
export default function ProjectPortfolioSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ProjectPortfolio', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ProjectPortfolio', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ProjectPortfolio-Schedule">
      <header>
        <h1>ProjectPortfolio / Schedule</h1>
