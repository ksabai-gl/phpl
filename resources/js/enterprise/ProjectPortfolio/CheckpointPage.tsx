...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/projectportfolio/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'ProjectPortfolio-Checkpoint-' + (index + 1),
...
export default function ProjectPortfolioCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ProjectPortfolio', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ProjectPortfolio', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ProjectPortfolio-Checkpoint">
      <header>
        <h1>ProjectPortfolio / Checkpoint</h1>
