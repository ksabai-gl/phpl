...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/projectportfolio/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'ProjectPortfolio-Metric-' + (index + 1),
...
export default function ProjectPortfolioMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ProjectPortfolio', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ProjectPortfolio', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ProjectPortfolio-Metric">
      <header>
        <h1>ProjectPortfolio / Metric</h1>
