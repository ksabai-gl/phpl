...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/secretsvault/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'SecretsVault-Metric-' + (index + 1),
...
export default function SecretsVaultMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SecretsVault', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SecretsVault', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SecretsVault-Metric">
      <header>
        <h1>SecretsVault / Metric</h1>
