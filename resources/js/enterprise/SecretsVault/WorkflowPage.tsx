...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/secretsvault/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'SecretsVault-Workflow-' + (index + 1),
...
export default function SecretsVaultWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SecretsVault', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SecretsVault', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SecretsVault-Workflow">
      <header>
        <h1>SecretsVault / Workflow</h1>
