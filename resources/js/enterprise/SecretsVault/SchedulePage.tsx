...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/secretsvault/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'SecretsVault-Schedule-' + (index + 1),
...
export default function SecretsVaultSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SecretsVault', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SecretsVault', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SecretsVault-Schedule">
      <header>
        <h1>SecretsVault / Schedule</h1>
