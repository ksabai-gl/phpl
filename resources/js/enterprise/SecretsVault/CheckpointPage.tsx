...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/secretsvault/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'SecretsVault-Checkpoint-' + (index + 1),
...
export default function SecretsVaultCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SecretsVault', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SecretsVault', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SecretsVault-Checkpoint">
      <header>
        <h1>SecretsVault / Checkpoint</h1>
