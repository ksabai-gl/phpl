...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/workflowengine/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'WorkflowEngine-Record-' + (index + 1),
...
export default function WorkflowEngineRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WorkflowEngine', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WorkflowEngine', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WorkflowEngine-Record">
      <header>
        <h1>WorkflowEngine / Record</h1>
