...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/workflowengine/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'WorkflowEngine-Item-' + (index + 1),
...
export default function WorkflowEngineItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WorkflowEngine', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WorkflowEngine', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WorkflowEngine-Item">
      <header>
        <h1>WorkflowEngine / Item</h1>
