...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/workflowengine/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'WorkflowEngine-Schedule-' + (index + 1),
...
export default function WorkflowEngineSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WorkflowEngine', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WorkflowEngine', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WorkflowEngine-Schedule">
      <header>
        <h1>WorkflowEngine / Schedule</h1>
