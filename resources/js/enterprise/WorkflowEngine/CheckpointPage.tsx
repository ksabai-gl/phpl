...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/workflowengine/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'WorkflowEngine-Checkpoint-' + (index + 1),
...
export default function WorkflowEngineCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WorkflowEngine', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WorkflowEngine', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WorkflowEngine-Checkpoint">
      <header>
        <h1>WorkflowEngine / Checkpoint</h1>
