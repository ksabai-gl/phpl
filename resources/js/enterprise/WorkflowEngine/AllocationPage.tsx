...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/workflowengine/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'WorkflowEngine-Allocation-' + (index + 1),
...
export default function WorkflowEngineAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WorkflowEngine', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WorkflowEngine', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WorkflowEngine-Allocation">
      <header>
        <h1>WorkflowEngine / Allocation</h1>
