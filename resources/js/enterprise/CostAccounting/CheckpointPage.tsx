...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/costaccounting/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'CostAccounting-Checkpoint-' + (index + 1),
...
export default function CostAccountingCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CostAccounting', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CostAccounting', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CostAccounting-Checkpoint">
      <header>
        <h1>CostAccounting / Checkpoint</h1>
