...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/costaccounting/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'CostAccounting-Schedule-' + (index + 1),
...
export default function CostAccountingSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CostAccounting', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CostAccounting', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CostAccounting-Schedule">
      <header>
        <h1>CostAccounting / Schedule</h1>
