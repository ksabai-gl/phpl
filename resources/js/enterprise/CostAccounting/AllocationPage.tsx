...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/costaccounting/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'CostAccounting-Allocation-' + (index + 1),
...
export default function CostAccountingAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CostAccounting', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CostAccounting', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CostAccounting-Allocation">
      <header>
        <h1>CostAccounting / Allocation</h1>
