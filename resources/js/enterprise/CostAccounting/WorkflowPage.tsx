...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/costaccounting/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'CostAccounting-Workflow-' + (index + 1),
...
export default function CostAccountingWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CostAccounting', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CostAccounting', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CostAccounting-Workflow">
      <header>
        <h1>CostAccounting / Workflow</h1>
