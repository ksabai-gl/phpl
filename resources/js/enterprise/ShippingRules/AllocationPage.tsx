...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/shippingrules/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'ShippingRules-Allocation-' + (index + 1),
...
export default function ShippingRulesAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ShippingRules', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ShippingRules', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ShippingRules-Allocation">
      <header>
        <h1>ShippingRules / Allocation</h1>
