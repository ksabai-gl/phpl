...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/shippingrules/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'ShippingRules-Checkpoint-' + (index + 1),
...
export default function ShippingRulesCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ShippingRules', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ShippingRules', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ShippingRules-Checkpoint">
      <header>
        <h1>ShippingRules / Checkpoint</h1>
