...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/shippingrules/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'ShippingRules-Schedule-' + (index + 1),
...
export default function ShippingRulesSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ShippingRules', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ShippingRules', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ShippingRules-Schedule">
      <header>
        <h1>ShippingRules / Schedule</h1>
