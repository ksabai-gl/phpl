...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/knowledgebase/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'KnowledgeBase-Item-' + (index + 1),
...
export default function KnowledgeBaseItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['KnowledgeBase', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['KnowledgeBase', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module KnowledgeBase-Item">
      <header>
        <h1>KnowledgeBase / Item</h1>
