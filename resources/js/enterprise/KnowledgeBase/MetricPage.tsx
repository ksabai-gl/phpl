...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/knowledgebase/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'KnowledgeBase-Metric-' + (index + 1),
...
export default function KnowledgeBaseMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['KnowledgeBase', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['KnowledgeBase', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module KnowledgeBase-Metric">
      <header>
        <h1>KnowledgeBase / Metric</h1>
