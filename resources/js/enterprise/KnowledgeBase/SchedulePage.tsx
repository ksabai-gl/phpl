...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/knowledgebase/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'KnowledgeBase-Schedule-' + (index + 1),
...
export default function KnowledgeBaseSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['KnowledgeBase', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['KnowledgeBase', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module KnowledgeBase-Schedule">
      <header>
        <h1>KnowledgeBase / Schedule</h1>
