...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/knowledgebase/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'KnowledgeBase-Record-' + (index + 1),
...
export default function KnowledgeBaseRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['KnowledgeBase', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['KnowledgeBase', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module KnowledgeBase-Record">
      <header>
        <h1>KnowledgeBase / Record</h1>
