...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/knowledgebase/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'KnowledgeBase-Checkpoint-' + (index + 1),
...
export default function KnowledgeBaseCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['KnowledgeBase', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['KnowledgeBase', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module KnowledgeBase-Checkpoint">
      <header>
        <h1>KnowledgeBase / Checkpoint</h1>
