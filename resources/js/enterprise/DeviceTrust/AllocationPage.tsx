...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/devicetrust/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'DeviceTrust-Allocation-' + (index + 1),
...
export default function DeviceTrustAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DeviceTrust', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DeviceTrust', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DeviceTrust-Allocation">
      <header>
        <h1>DeviceTrust / Allocation</h1>
