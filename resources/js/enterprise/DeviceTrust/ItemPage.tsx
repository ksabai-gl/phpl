...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/devicetrust/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'DeviceTrust-Item-' + (index + 1),
...
export default function DeviceTrustItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DeviceTrust', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DeviceTrust', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DeviceTrust-Item">
      <header>
        <h1>DeviceTrust / Item</h1>
