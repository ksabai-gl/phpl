...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/devicetrust/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'DeviceTrust-Workflow-' + (index + 1),
...
export default function DeviceTrustWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DeviceTrust', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DeviceTrust', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DeviceTrust-Workflow">
      <header>
        <h1>DeviceTrust / Workflow</h1>
