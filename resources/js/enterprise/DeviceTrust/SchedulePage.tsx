...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/devicetrust/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'DeviceTrust-Schedule-' + (index + 1),
...
export default function DeviceTrustSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DeviceTrust', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DeviceTrust', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DeviceTrust-Schedule">
      <header>
        <h1>DeviceTrust / Schedule</h1>
