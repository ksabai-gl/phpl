...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/devicetrust/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'DeviceTrust-Record-' + (index + 1),
...
export default function DeviceTrustRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DeviceTrust', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DeviceTrust', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DeviceTrust-Record">
      <header>
        <h1>DeviceTrust / Record</h1>
