...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/identityaccess/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'IdentityAccess-Schedule-' + (index + 1),
...
export default function IdentityAccessSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IdentityAccess', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IdentityAccess', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IdentityAccess-Schedule">
      <header>
        <h1>IdentityAccess / Schedule</h1>
