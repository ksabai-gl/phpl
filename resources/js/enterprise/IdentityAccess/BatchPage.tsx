...
async function fetchBatchRows(): Promise<BatchRow[]> {
  const response = await fetch('/api/v1/enterprise/identityaccess/batch');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Batch ' + (index + 1),
      code: 'IdentityAccess-Batch-' + (index + 1),
...
export default function IdentityAccessBatchPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IdentityAccess', 'Batch', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IdentityAccess', 'Batch'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IdentityAccess-Batch">
      <header>
        <h1>IdentityAccess / Batch</h1>
