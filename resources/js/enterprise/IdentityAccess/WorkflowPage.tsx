...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/identityaccess/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'IdentityAccess-Workflow-' + (index + 1),
...
export default function IdentityAccessWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IdentityAccess', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IdentityAccess', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IdentityAccess-Workflow">
      <header>
        <h1>IdentityAccess / Workflow</h1>
