...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/identityaccess/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'IdentityAccess-Allocation-' + (index + 1),
...
export default function IdentityAccessAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IdentityAccess', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IdentityAccess', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IdentityAccess-Allocation">
      <header>
        <h1>IdentityAccess / Allocation</h1>
