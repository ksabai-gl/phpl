...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/identityaccess/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'IdentityAccess-Metric-' + (index + 1),
...
export default function IdentityAccessMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IdentityAccess', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IdentityAccess', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IdentityAccess-Metric">
      <header>
        <h1>IdentityAccess / Metric</h1>
