...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/dataimporthub/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'DataImportHub-Allocation-' + (index + 1),
...
export default function DataImportHubAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DataImportHub', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DataImportHub', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DataImportHub-Allocation">
      <header>
        <h1>DataImportHub / Allocation</h1>
