...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/dataimporthub/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'DataImportHub-Metric-' + (index + 1),
...
export default function DataImportHubMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DataImportHub', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DataImportHub', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DataImportHub-Metric">
      <header>
        <h1>DataImportHub / Metric</h1>
