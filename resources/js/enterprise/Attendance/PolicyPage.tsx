...
async function fetchPolicyRows(): Promise<PolicyRow[]> {
  const response = await fetch('/api/v1/enterprise/attendance/policy');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Policy ' + (index + 1),
      code: 'Attendance-Policy-' + (index + 1),
...
export default function AttendancePolicyPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['Attendance', 'Policy', filter, status],
  ...
  return (
    <section className="enterprise-module Attendance-Policy">
      <header>
