...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/sessiongovernance/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'SessionGovernance-Record-' + (index + 1),
...
export default function SessionGovernanceRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SessionGovernance', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SessionGovernance', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SessionGovernance-Record">
      <header>
        <h1>SessionGovernance / Record</h1>
