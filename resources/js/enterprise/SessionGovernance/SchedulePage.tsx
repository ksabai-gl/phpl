...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/sessiongovernance/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'SessionGovernance-Schedule-' + (index + 1),
...
export default function SessionGovernanceSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SessionGovernance', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SessionGovernance', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SessionGovernance-Schedule">
      <header>
        <h1>SessionGovernance / Schedule</h1>
