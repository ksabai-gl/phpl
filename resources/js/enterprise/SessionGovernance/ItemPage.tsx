...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/sessiongovernance/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'SessionGovernance-Item-' + (index + 1),
...
export default function SessionGovernanceItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SessionGovernance', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SessionGovernance', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SessionGovernance-Item">
      <header>
        <h1>SessionGovernance / Item</h1>
