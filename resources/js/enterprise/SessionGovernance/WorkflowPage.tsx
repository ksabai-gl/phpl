...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/sessiongovernance/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'SessionGovernance-Workflow-' + (index + 1),
...
export default function SessionGovernanceWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SessionGovernance', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SessionGovernance', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SessionGovernance-Workflow">
      <header>
        <h1>SessionGovernance / Workflow</h1>
