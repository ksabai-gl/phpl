...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/sessiongovernance/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'SessionGovernance-Allocation-' + (index + 1),
...
export default function SessionGovernanceAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SessionGovernance', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SessionGovernance', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SessionGovernance-Allocation">
      <header>
        <h1>SessionGovernance / Allocation</h1>
