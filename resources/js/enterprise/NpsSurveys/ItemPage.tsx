...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/npssurveys/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'NpsSurveys-Item-' + (index + 1),
...
export default function NpsSurveysItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['NpsSurveys', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['NpsSurveys', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module NpsSurveys-Item">
      <header>
        <h1>NpsSurveys / Item</h1>
