...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/npssurveys/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'NpsSurveys-Allocation-' + (index + 1),
...
export default function NpsSurveysAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['NpsSurveys', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['NpsSurveys', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module NpsSurveys-Allocation">
      <header>
        <h1>NpsSurveys / Allocation</h1>
