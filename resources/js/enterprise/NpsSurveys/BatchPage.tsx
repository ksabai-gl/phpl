...
async function fetchBatchRows(): Promise<BatchRow[]> {
  const response = await fetch('/api/v1/enterprise/npssurveys/batch');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Batch ' + (index + 1),
      code: 'NpsSurveys-Batch-' + (index + 1),
...
export default function NpsSurveysBatchPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['NpsSurveys', 'Batch', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['NpsSurveys', 'Batch'] });
    },
  });
  ...
  return (
    <section className="enterprise-module NpsSurveys-Batch">
      <header>
        <h1>NpsSurveys / Batch</h1>
