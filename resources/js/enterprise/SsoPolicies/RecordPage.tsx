...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/ssopolicies/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'SsoPolicies-Record-' + (index + 1),
...
export default function SsoPoliciesRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SsoPolicies', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SsoPolicies', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SsoPolicies-Record">
      <header>
        <h1>SsoPolicies / Record</h1>
