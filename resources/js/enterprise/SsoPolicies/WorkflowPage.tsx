...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/ssopolicies/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'SsoPolicies-Workflow-' + (index + 1),
...
export default function SsoPoliciesWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SsoPolicies', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SsoPolicies', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SsoPolicies-Workflow">
      <header>
        <h1>SsoPolicies / Workflow</h1>
