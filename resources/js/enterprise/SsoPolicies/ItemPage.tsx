...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/ssopolicies/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'SsoPolicies-Item-' + (index + 1),
...
export default function SsoPoliciesItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SsoPolicies', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SsoPolicies', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SsoPolicies-Item">
      <header>
        <h1>SsoPolicies / Item</h1>
