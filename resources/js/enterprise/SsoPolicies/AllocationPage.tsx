...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/ssopolicies/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'SsoPolicies-Allocation-' + (index + 1),
...
export default function SsoPoliciesAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SsoPolicies', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SsoPolicies', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SsoPolicies-Allocation">
      <header>
        <h1>SsoPolicies / Allocation</h1>
