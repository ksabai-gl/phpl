...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/ssopolicies/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'SsoPolicies-Metric-' + (index + 1),
...
export default function SsoPoliciesMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SsoPolicies', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SsoPolicies', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SsoPolicies-Metric">
      <header>
        <h1>SsoPolicies / Metric</h1>
