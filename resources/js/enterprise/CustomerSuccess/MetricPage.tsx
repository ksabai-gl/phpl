...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/customersuccess/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'CustomerSuccess-Metric-' + (index + 1),
...
export default function CustomerSuccessMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CustomerSuccess', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CustomerSuccess', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CustomerSuccess-Metric">
      <header>
        <h1>CustomerSuccess / Metric</h1>
