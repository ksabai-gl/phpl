...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/customersuccess/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'CustomerSuccess-Workflow-' + (index + 1),
...
export default function CustomerSuccessWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CustomerSuccess', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CustomerSuccess', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CustomerSuccess-Workflow">
      <header>
        <h1>CustomerSuccess / Workflow</h1>
