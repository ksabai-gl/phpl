...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/customersuccess/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'CustomerSuccess-Schedule-' + (index + 1),
...
export default function CustomerSuccessSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CustomerSuccess', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CustomerSuccess', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CustomerSuccess-Schedule">
      <header>
        <h1>CustomerSuccess / Schedule</h1>
