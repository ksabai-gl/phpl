...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/customersuccess/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'CustomerSuccess-Record-' + (index + 1),
...
export default function CustomerSuccessRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CustomerSuccess', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CustomerSuccess', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CustomerSuccess-Record">
      <header>
        <h1>CustomerSuccess / Record</h1>
