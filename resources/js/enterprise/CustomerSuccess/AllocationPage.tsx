...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/customersuccess/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'CustomerSuccess-Allocation-' + (index + 1),
...
export default function CustomerSuccessAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CustomerSuccess', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CustomerSuccess', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CustomerSuccess-Allocation">
      <header>
        <h1>CustomerSuccess / Allocation</h1>
