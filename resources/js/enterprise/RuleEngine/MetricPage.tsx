...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/ruleengine/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'RuleEngine-Metric-' + (index + 1),
...
export default function RuleEngineMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['RuleEngine', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['RuleEngine', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module RuleEngine-Metric">
      <header>
        <h1>RuleEngine / Metric</h1>
