...
async function fetchBatchRows(): Promise<BatchRow[]> {
  const response = await fetch('/api/v1/enterprise/ruleengine/batch');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Batch ' + (index + 1),
      code: 'RuleEngine-Batch-' + (index + 1),
...
export default function RuleEngineBatchPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['RuleEngine', 'Batch', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['RuleEngine', 'Batch'] });
    },
  });
  ...
  return (
    <section className="enterprise-module RuleEngine-Batch">
      <header>
        <h1>RuleEngine / Batch</h1>
