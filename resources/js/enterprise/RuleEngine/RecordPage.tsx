...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/ruleengine/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'RuleEngine-Record-' + (index + 1),
...
export default function RuleEngineRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['RuleEngine', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['RuleEngine', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module RuleEngine-Record">
      <header>
        <h1>RuleEngine / Record</h1>
