...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/ruleengine/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'RuleEngine-Workflow-' + (index + 1),
...
export default function RuleEngineWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['RuleEngine', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['RuleEngine', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module RuleEngine-Workflow">
      <header>
        <h1>RuleEngine / Workflow</h1>
