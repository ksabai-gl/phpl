...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/ruleengine/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'RuleEngine-Allocation-' + (index + 1),
...
export default function RuleEngineAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['RuleEngine', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['RuleEngine', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module RuleEngine-Allocation">
      <header>
        <h1>RuleEngine / Allocation</h1>
