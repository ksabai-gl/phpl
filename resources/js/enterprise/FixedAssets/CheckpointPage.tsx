...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/fixedassets/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'FixedAssets-Checkpoint-' + (index + 1),
...
export default function FixedAssetsCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FixedAssets', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FixedAssets', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FixedAssets-Checkpoint">
      <header>
        <h1>FixedAssets / Checkpoint</h1>
