...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/fixedassets/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'FixedAssets-Metric-' + (index + 1),
...
export default function FixedAssetsMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FixedAssets', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FixedAssets', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FixedAssets-Metric">
      <header>
        <h1>FixedAssets / Metric</h1>
