...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/fixedassets/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'FixedAssets-Allocation-' + (index + 1),
...
export default function FixedAssetsAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FixedAssets', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FixedAssets', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FixedAssets-Allocation">
      <header>
        <h1>FixedAssets / Allocation</h1>
