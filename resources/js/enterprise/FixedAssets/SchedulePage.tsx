...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/fixedassets/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'FixedAssets-Schedule-' + (index + 1),
...
export default function FixedAssetsSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FixedAssets', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FixedAssets', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FixedAssets-Schedule">
      <header>
        <h1>FixedAssets / Schedule</h1>
