...
async function fetchPolicyRows(): Promise<PolicyRow[]> {
  const response = await fetch('/api/v1/enterprise/compliance/policy');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Policy ' + (index + 1),
      code: 'Compliance-Policy-' + (index + 1),
...
export default function CompliancePolicyPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['Compliance', 'Policy', filter, status],
  ...
  return (
    <section className="enterprise-module Compliance-Policy">
      <header>
