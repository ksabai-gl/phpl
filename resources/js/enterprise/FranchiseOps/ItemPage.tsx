...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/franchiseops/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'FranchiseOps-Item-' + (index + 1),
...
export default function FranchiseOpsItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FranchiseOps', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FranchiseOps', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FranchiseOps-Item">
      <header>
        <h1>FranchiseOps / Item</h1>
