...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/franchiseops/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'FranchiseOps-Allocation-' + (index + 1),
...
export default function FranchiseOpsAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FranchiseOps', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FranchiseOps', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FranchiseOps-Allocation">
      <header>
        <h1>FranchiseOps / Allocation</h1>
