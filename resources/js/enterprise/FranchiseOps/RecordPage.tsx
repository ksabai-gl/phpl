...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/franchiseops/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'FranchiseOps-Record-' + (index + 1),
...
export default function FranchiseOpsRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FranchiseOps', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FranchiseOps', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FranchiseOps-Record">
      <header>
        <h1>FranchiseOps / Record</h1>
