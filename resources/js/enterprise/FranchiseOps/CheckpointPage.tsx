...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/franchiseops/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'FranchiseOps-Checkpoint-' + (index + 1),
...
export default function FranchiseOpsCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FranchiseOps', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FranchiseOps', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FranchiseOps-Checkpoint">
      <header>
        <h1>FranchiseOps / Checkpoint</h1>
