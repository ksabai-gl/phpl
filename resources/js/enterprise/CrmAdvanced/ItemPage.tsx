...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/crmadvanced/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'CrmAdvanced-Item-' + (index + 1),
...
export default function CrmAdvancedItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CrmAdvanced', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CrmAdvanced', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CrmAdvanced-Item">
      <header>
        <h1>CrmAdvanced / Item</h1>
