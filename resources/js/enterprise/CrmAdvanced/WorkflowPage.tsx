...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/crmadvanced/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'CrmAdvanced-Workflow-' + (index + 1),
...
export default function CrmAdvancedWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CrmAdvanced', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CrmAdvanced', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CrmAdvanced-Workflow">
      <header>
        <h1>CrmAdvanced / Workflow</h1>
