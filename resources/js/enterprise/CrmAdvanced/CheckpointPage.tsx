...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/crmadvanced/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'CrmAdvanced-Checkpoint-' + (index + 1),
...
export default function CrmAdvancedCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CrmAdvanced', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CrmAdvanced', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CrmAdvanced-Checkpoint">
      <header>
        <h1>CrmAdvanced / Checkpoint</h1>
