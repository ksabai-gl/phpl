...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/crmadvanced/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'CrmAdvanced-Record-' + (index + 1),
...
export default function CrmAdvancedRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CrmAdvanced', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CrmAdvanced', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CrmAdvanced-Record">
      <header>
        <h1>CrmAdvanced / Record</h1>
