...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/crmadvanced/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'CrmAdvanced-Allocation-' + (index + 1),
...
export default function CrmAdvancedAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CrmAdvanced', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CrmAdvanced', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CrmAdvanced-Allocation">
      <header>
        <h1>CrmAdvanced / Allocation</h1>
