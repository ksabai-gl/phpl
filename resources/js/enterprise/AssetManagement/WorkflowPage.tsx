...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/assetmanagement/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'AssetManagement-Workflow-' + (index + 1),
...
export default function AssetManagementWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['AssetManagement', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['AssetManagement', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module AssetManagement-Workflow">
      <header>
        <h1>AssetManagement / Workflow</h1>
