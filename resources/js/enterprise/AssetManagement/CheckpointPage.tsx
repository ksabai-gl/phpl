...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/assetmanagement/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'AssetManagement-Checkpoint-' + (index + 1),
...
export default function AssetManagementCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['AssetManagement', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['AssetManagement', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module AssetManagement-Checkpoint">
      <header>
        <h1>AssetManagement / Checkpoint</h1>
