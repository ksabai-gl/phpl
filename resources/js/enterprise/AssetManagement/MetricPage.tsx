...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/assetmanagement/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'AssetManagement-Metric-' + (index + 1),
...
export default function AssetManagementMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['AssetManagement', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['AssetManagement', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module AssetManagement-Metric">
      <header>
        <h1>AssetManagement / Metric</h1>
