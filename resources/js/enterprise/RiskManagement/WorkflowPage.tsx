...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/riskmanagement/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'RiskManagement-Workflow-' + (index + 1),
...
export default function RiskManagementWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['RiskManagement', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['RiskManagement', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module RiskManagement-Workflow">
      <header>
        <h1>RiskManagement / Workflow</h1>
