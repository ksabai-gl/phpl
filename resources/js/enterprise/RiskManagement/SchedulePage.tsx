...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/riskmanagement/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'RiskManagement-Schedule-' + (index + 1),
...
export default function RiskManagementSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['RiskManagement', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['RiskManagement', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module RiskManagement-Schedule">
      <header>
        <h1>RiskManagement / Schedule</h1>
