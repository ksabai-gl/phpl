...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/riskmanagement/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'RiskManagement-Allocation-' + (index + 1),
...
export default function RiskManagementAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['RiskManagement', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['RiskManagement', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module RiskManagement-Allocation">
      <header>
        <h1>RiskManagement / Allocation</h1>
