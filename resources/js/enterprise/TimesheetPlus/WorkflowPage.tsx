...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/timesheetplus/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'TimesheetPlus-Workflow-' + (index + 1),
...
export default function TimesheetPlusWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['TimesheetPlus', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['TimesheetPlus', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module TimesheetPlus-Workflow">
      <header>
        <h1>TimesheetPlus / Workflow</h1>
