...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/timesheetplus/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'TimesheetPlus-Allocation-' + (index + 1),
...
export default function TimesheetPlusAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['TimesheetPlus', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['TimesheetPlus', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module TimesheetPlus-Allocation">
      <header>
        <h1>TimesheetPlus / Allocation</h1>
