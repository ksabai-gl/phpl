...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/timesheetplus/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'TimesheetPlus-Item-' + (index + 1),
...
export default function TimesheetPlusItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['TimesheetPlus', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['TimesheetPlus', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module TimesheetPlus-Item">
      <header>
        <h1>TimesheetPlus / Item</h1>
