...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/timesheetplus/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'TimesheetPlus-Record-' + (index + 1),
...
export default function TimesheetPlusRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['TimesheetPlus', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['TimesheetPlus', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module TimesheetPlus-Record">
      <header>
        <h1>TimesheetPlus / Record</h1>
