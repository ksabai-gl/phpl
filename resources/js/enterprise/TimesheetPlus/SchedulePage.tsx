...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/timesheetplus/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'TimesheetPlus-Schedule-' + (index + 1),
...
export default function TimesheetPlusSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['TimesheetPlus', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['TimesheetPlus', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module TimesheetPlus-Schedule">
      <header>
        <h1>TimesheetPlus / Schedule</h1>
