...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/analyticshub/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'AnalyticsHub-Metric-' + (index + 1),
...
export default function AnalyticsHubMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['AnalyticsHub', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['AnalyticsHub', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module AnalyticsHub-Metric">
      <header>
        <h1>AnalyticsHub / Metric</h1>
