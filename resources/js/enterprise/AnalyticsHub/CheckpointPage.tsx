...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/analyticshub/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'AnalyticsHub-Checkpoint-' + (index + 1),
...
export default function AnalyticsHubCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['AnalyticsHub', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['AnalyticsHub', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module AnalyticsHub-Checkpoint">
      <header>
        <h1>AnalyticsHub / Checkpoint</h1>
