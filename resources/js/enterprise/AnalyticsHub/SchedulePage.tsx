...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/analyticshub/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'AnalyticsHub-Schedule-' + (index + 1),
...
export default function AnalyticsHubSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['AnalyticsHub', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['AnalyticsHub', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module AnalyticsHub-Schedule">
      <header>
        <h1>AnalyticsHub / Schedule</h1>
