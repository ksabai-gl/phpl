...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/analyticshub/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'AnalyticsHub-Workflow-' + (index + 1),
...
export default function AnalyticsHubWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['AnalyticsHub', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['AnalyticsHub', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module AnalyticsHub-Workflow">
      <header>
        <h1>AnalyticsHub / Workflow</h1>
