...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/analyticshub/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'AnalyticsHub-Record-' + (index + 1),
...
export default function AnalyticsHubRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['AnalyticsHub', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['AnalyticsHub', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module AnalyticsHub-Record">
      <header>
        <h1>AnalyticsHub / Record</h1>
