...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/audittrail/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'AuditTrail-Schedule-' + (index + 1),
...
export default function AuditTrailSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['AuditTrail', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['AuditTrail', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module AuditTrail-Schedule">
      <header>
        <h1>AuditTrail / Schedule</h1>
