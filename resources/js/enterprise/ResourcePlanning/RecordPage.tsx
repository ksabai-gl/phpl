...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/resourceplanning/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'ResourcePlanning-Record-' + (index + 1),
...
export default function ResourcePlanningRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ResourcePlanning', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ResourcePlanning', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ResourcePlanning-Record">
      <header>
        <h1>ResourcePlanning / Record</h1>
