...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/resourceplanning/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'ResourcePlanning-Workflow-' + (index + 1),
...
export default function ResourcePlanningWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ResourcePlanning', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ResourcePlanning', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ResourcePlanning-Workflow">
      <header>
        <h1>ResourcePlanning / Workflow</h1>
