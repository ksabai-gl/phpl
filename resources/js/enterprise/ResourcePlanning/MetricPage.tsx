...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/resourceplanning/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'ResourcePlanning-Metric-' + (index + 1),
...
export default function ResourcePlanningMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ResourcePlanning', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ResourcePlanning', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ResourcePlanning-Metric">
      <header>
        <h1>ResourcePlanning / Metric</h1>
