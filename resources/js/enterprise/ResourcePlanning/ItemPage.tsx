...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/resourceplanning/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'ResourcePlanning-Item-' + (index + 1),
...
export default function ResourcePlanningItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ResourcePlanning', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ResourcePlanning', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ResourcePlanning-Item">
      <header>
        <h1>ResourcePlanning / Item</h1>
