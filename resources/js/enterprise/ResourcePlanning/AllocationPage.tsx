...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/resourceplanning/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'ResourcePlanning-Allocation-' + (index + 1),
...
export default function ResourcePlanningAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ResourcePlanning', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ResourcePlanning', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ResourcePlanning-Allocation">
      <header>
        <h1>ResourcePlanning / Allocation</h1>
