...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/integrationhub/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'IntegrationHub-Record-' + (index + 1),
...
export default function IntegrationHubRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IntegrationHub', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IntegrationHub', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IntegrationHub-Record">
      <header>
        <h1>IntegrationHub / Record</h1>
