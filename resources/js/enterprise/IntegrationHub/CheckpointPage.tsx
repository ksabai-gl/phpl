...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/integrationhub/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'IntegrationHub-Checkpoint-' + (index + 1),
...
export default function IntegrationHubCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IntegrationHub', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IntegrationHub', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IntegrationHub-Checkpoint">
      <header>
        <h1>IntegrationHub / Checkpoint</h1>
