...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/integrationhub/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'IntegrationHub-Allocation-' + (index + 1),
...
export default function IntegrationHubAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IntegrationHub', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IntegrationHub', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IntegrationHub-Allocation">
      <header>
        <h1>IntegrationHub / Allocation</h1>
