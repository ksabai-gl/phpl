...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/integrationhub/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'IntegrationHub-Metric-' + (index + 1),
...
export default function IntegrationHubMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IntegrationHub', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IntegrationHub', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IntegrationHub-Metric">
      <header>
        <h1>IntegrationHub / Metric</h1>
