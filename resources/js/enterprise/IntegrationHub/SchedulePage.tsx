...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/integrationhub/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'IntegrationHub-Schedule-' + (index + 1),
...
export default function IntegrationHubSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IntegrationHub', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IntegrationHub', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IntegrationHub-Schedule">
      <header>
        <h1>IntegrationHub / Schedule</h1>
