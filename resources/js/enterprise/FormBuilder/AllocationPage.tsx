...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/formbuilder/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'FormBuilder-Allocation-' + (index + 1),
...
export default function FormBuilderAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FormBuilder', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FormBuilder', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FormBuilder-Allocation">
      <header>
        <h1>FormBuilder / Allocation</h1>
