...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/formbuilder/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'FormBuilder-Workflow-' + (index + 1),
...
export default function FormBuilderWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FormBuilder', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FormBuilder', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FormBuilder-Workflow">
      <header>
        <h1>FormBuilder / Workflow</h1>
