...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/formbuilder/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'FormBuilder-Schedule-' + (index + 1),
...
export default function FormBuilderSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FormBuilder', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FormBuilder', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FormBuilder-Schedule">
      <header>
        <h1>FormBuilder / Schedule</h1>
