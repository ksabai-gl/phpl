...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/formbuilder/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'FormBuilder-Metric-' + (index + 1),
...
export default function FormBuilderMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FormBuilder', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FormBuilder', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FormBuilder-Metric">
      <header>
        <h1>FormBuilder / Metric</h1>
