...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/marketplacesync/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'MarketplaceSync-Allocation-' + (index + 1),
...
export default function MarketplaceSyncAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['MarketplaceSync', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['MarketplaceSync', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module MarketplaceSync-Allocation">
      <header>
        <h1>MarketplaceSync / Allocation</h1>
