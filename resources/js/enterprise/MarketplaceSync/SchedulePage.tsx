...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/marketplacesync/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'MarketplaceSync-Schedule-' + (index + 1),
...
export default function MarketplaceSyncSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['MarketplaceSync', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['MarketplaceSync', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module MarketplaceSync-Schedule">
      <header>
        <h1>MarketplaceSync / Schedule</h1>
