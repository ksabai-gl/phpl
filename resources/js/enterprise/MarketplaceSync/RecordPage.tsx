...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/marketplacesync/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'MarketplaceSync-Record-' + (index + 1),
...
export default function MarketplaceSyncRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['MarketplaceSync', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['MarketplaceSync', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module MarketplaceSync-Record">
      <header>
        <h1>MarketplaceSync / Record</h1>
