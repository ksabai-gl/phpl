...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/marketplacesync/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'MarketplaceSync-Checkpoint-' + (index + 1),
...
export default function MarketplaceSyncCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['MarketplaceSync', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['MarketplaceSync', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module MarketplaceSync-Checkpoint">
      <header>
        <h1>MarketplaceSync / Checkpoint</h1>
