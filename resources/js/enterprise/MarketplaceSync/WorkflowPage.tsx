...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/marketplacesync/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'MarketplaceSync-Workflow-' + (index + 1),
...
export default function MarketplaceSyncWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['MarketplaceSync', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['MarketplaceSync', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module MarketplaceSync-Workflow">
      <header>
        <h1>MarketplaceSync / Workflow</h1>
