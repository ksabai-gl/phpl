...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/notificationcenter/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'NotificationCenter-Record-' + (index + 1),
...
export default function NotificationCenterRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['NotificationCenter', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['NotificationCenter', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module NotificationCenter-Record">
      <header>
        <h1>NotificationCenter / Record</h1>
