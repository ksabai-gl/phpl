...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/notificationcenter/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'NotificationCenter-Allocation-' + (index + 1),
...
export default function NotificationCenterAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['NotificationCenter', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['NotificationCenter', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module NotificationCenter-Allocation">
      <header>
        <h1>NotificationCenter / Allocation</h1>
