...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/notificationcenter/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'NotificationCenter-Workflow-' + (index + 1),
...
export default function NotificationCenterWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['NotificationCenter', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['NotificationCenter', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module NotificationCenter-Workflow">
      <header>
        <h1>NotificationCenter / Workflow</h1>
