...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/notificationcenter/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'NotificationCenter-Metric-' + (index + 1),
...
export default function NotificationCenterMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['NotificationCenter', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['NotificationCenter', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module NotificationCenter-Metric">
      <header>
        <h1>NotificationCenter / Metric</h1>
