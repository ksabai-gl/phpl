...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/notificationcenter/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'NotificationCenter-Schedule-' + (index + 1),
...
export default function NotificationCenterSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['NotificationCenter', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['NotificationCenter', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module NotificationCenter-Schedule">
      <header>
        <h1>NotificationCenter / Schedule</h1>
