...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/dataexporthub/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'DataExportHub-Checkpoint-' + (index + 1),
...
export default function DataExportHubCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DataExportHub', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DataExportHub', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DataExportHub-Checkpoint">
      <header>
        <h1>DataExportHub / Checkpoint</h1>
