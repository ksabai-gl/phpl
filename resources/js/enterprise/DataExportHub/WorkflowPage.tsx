...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/dataexporthub/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'DataExportHub-Workflow-' + (index + 1),
...
export default function DataExportHubWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DataExportHub', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DataExportHub', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DataExportHub-Workflow">
      <header>
        <h1>DataExportHub / Workflow</h1>
