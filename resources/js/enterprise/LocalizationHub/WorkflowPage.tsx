...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/localizationhub/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'LocalizationHub-Workflow-' + (index + 1),
...
export default function LocalizationHubWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['LocalizationHub', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['LocalizationHub', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module LocalizationHub-Workflow">
      <header>
        <h1>LocalizationHub / Workflow</h1>
