...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/localizationhub/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'LocalizationHub-Checkpoint-' + (index + 1),
...
export default function LocalizationHubCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['LocalizationHub', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['LocalizationHub', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module LocalizationHub-Checkpoint">
      <header>
        <h1>LocalizationHub / Checkpoint</h1>
