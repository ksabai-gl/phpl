...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/localizationhub/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'LocalizationHub-Item-' + (index + 1),
...
export default function LocalizationHubItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['LocalizationHub', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['LocalizationHub', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module LocalizationHub-Item">
      <header>
        <h1>LocalizationHub / Item</h1>
