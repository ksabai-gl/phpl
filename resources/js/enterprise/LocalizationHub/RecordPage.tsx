...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/localizationhub/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'LocalizationHub-Record-' + (index + 1),
...
export default function LocalizationHubRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['LocalizationHub', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['LocalizationHub', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module LocalizationHub-Record">
      <header>
        <h1>LocalizationHub / Record</h1>
