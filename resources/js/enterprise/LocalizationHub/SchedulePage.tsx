...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/localizationhub/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'LocalizationHub-Schedule-' + (index + 1),
...
export default function LocalizationHubSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['LocalizationHub', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['LocalizationHub', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module LocalizationHub-Schedule">
      <header>
        <h1>LocalizationHub / Schedule</h1>
