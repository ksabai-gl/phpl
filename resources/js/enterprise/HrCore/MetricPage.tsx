...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/hrcore/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'HrCore-Metric-' + (index + 1),
...
export default function HrCoreMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['HrCore', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['HrCore', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module HrCore-Metric">
      <header>
        <h1>HrCore / Metric</h1>
