...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/cashflow/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'CashFlow-Allocation-' + (index + 1),
...
export default function CashFlowAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CashFlow', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CashFlow', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CashFlow-Allocation">
      <header>
        <h1>CashFlow / Allocation</h1>
