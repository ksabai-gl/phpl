...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/maintenanceplans/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'MaintenancePlans-Schedule-' + (index + 1),
...
export default function MaintenancePlansSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['MaintenancePlans', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['MaintenancePlans', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module MaintenancePlans-Schedule">
      <header>
        <h1>MaintenancePlans / Schedule</h1>
