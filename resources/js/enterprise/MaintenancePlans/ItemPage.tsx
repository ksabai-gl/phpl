...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/maintenanceplans/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'MaintenancePlans-Item-' + (index + 1),
...
export default function MaintenancePlansItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['MaintenancePlans', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['MaintenancePlans', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module MaintenancePlans-Item">
      <header>
        <h1>MaintenancePlans / Item</h1>
