...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/maintenanceplans/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'MaintenancePlans-Metric-' + (index + 1),
...
export default function MaintenancePlansMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['MaintenancePlans', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['MaintenancePlans', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module MaintenancePlans-Metric">
      <header>
        <h1>MaintenancePlans / Metric</h1>
