...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/maintenanceplans/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'MaintenancePlans-Record-' + (index + 1),
...
export default function MaintenancePlansRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['MaintenancePlans', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['MaintenancePlans', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module MaintenancePlans-Record">
      <header>
        <h1>MaintenancePlans / Record</h1>
