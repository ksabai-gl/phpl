...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/maintenanceplans/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'MaintenancePlans-Workflow-' + (index + 1),
...
export default function MaintenancePlansWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['MaintenancePlans', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['MaintenancePlans', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module MaintenancePlans-Workflow">
      <header>
        <h1>MaintenancePlans / Workflow</h1>
