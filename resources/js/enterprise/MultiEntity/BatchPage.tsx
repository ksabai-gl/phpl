...
async function fetchBatchRows(): Promise<BatchRow[]> {
  const response = await fetch('/api/v1/enterprise/multientity/batch');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Batch ' + (index + 1),
      code: 'MultiEntity-Batch-' + (index + 1),
...
export default function MultiEntityBatchPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['MultiEntity', 'Batch', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['MultiEntity', 'Batch'] });
    },
  });
  ...
  return (
    <section className="enterprise-module MultiEntity-Batch">
      <header>
        <h1>MultiEntity / Batch</h1>
