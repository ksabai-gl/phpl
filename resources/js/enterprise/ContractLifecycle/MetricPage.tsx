...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/contractlifecycle/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'ContractLifecycle-Metric-' + (index + 1),
...
export default function ContractLifecycleMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ContractLifecycle', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ContractLifecycle', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ContractLifecycle-Metric">
      <header>
        <h1>ContractLifecycle / Metric</h1>
