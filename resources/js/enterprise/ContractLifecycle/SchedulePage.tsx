...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/contractlifecycle/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'ContractLifecycle-Schedule-' + (index + 1),
...
export default function ContractLifecycleSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ContractLifecycle', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ContractLifecycle', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ContractLifecycle-Schedule">
      <header>
        <h1>ContractLifecycle / Schedule</h1>
