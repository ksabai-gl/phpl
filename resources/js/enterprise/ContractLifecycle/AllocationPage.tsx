...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/contractlifecycle/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'ContractLifecycle-Allocation-' + (index + 1),
...
export default function ContractLifecycleAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ContractLifecycle', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ContractLifecycle', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ContractLifecycle-Allocation">
      <header>
        <h1>ContractLifecycle / Allocation</h1>
