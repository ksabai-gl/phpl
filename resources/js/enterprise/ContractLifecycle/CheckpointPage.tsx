...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/contractlifecycle/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'ContractLifecycle-Checkpoint-' + (index + 1),
...
export default function ContractLifecycleCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ContractLifecycle', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ContractLifecycle', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ContractLifecycle-Checkpoint">
      <header>
        <h1>ContractLifecycle / Checkpoint</h1>
