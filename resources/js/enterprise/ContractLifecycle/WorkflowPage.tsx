...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/contractlifecycle/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'ContractLifecycle-Workflow-' + (index + 1),
...
export default function ContractLifecycleWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ContractLifecycle', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ContractLifecycle', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ContractLifecycle-Workflow">
      <header>
        <h1>ContractLifecycle / Workflow</h1>
