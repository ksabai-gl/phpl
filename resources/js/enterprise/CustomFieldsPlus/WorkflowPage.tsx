...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/customfieldsplus/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'CustomFieldsPlus-Workflow-' + (index + 1),
...
export default function CustomFieldsPlusWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CustomFieldsPlus', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CustomFieldsPlus', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CustomFieldsPlus-Workflow">
      <header>
        <h1>CustomFieldsPlus / Workflow</h1>
