...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/customfieldsplus/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'CustomFieldsPlus-Schedule-' + (index + 1),
...
export default function CustomFieldsPlusSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CustomFieldsPlus', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CustomFieldsPlus', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CustomFieldsPlus-Schedule">
      <header>
        <h1>CustomFieldsPlus / Schedule</h1>
