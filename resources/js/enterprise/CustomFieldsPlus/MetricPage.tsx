...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/customfieldsplus/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'CustomFieldsPlus-Metric-' + (index + 1),
...
export default function CustomFieldsPlusMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CustomFieldsPlus', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CustomFieldsPlus', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CustomFieldsPlus-Metric">
      <header>
        <h1>CustomFieldsPlus / Metric</h1>
