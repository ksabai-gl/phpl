...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/customfieldsplus/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'CustomFieldsPlus-Item-' + (index + 1),
...
export default function CustomFieldsPlusItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CustomFieldsPlus', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CustomFieldsPlus', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CustomFieldsPlus-Item">
      <header>
        <h1>CustomFieldsPlus / Item</h1>
