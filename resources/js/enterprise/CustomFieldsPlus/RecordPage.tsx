...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/customfieldsplus/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'CustomFieldsPlus-Record-' + (index + 1),
...
export default function CustomFieldsPlusRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CustomFieldsPlus', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CustomFieldsPlus', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CustomFieldsPlus-Record">
      <header>
        <h1>CustomFieldsPlus / Record</h1>
