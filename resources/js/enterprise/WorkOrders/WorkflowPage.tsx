...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/workorders/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'WorkOrders-Workflow-' + (index + 1),
...
export default function WorkOrdersWorkflowPage(): React.JSX.Element {
  ...
  const query = useQuery({
    queryKey: ['WorkOrders', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WorkOrders', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WorkOrders-Workflow">
      <header>
        <h1>WorkOrders / Workflow</h1>
