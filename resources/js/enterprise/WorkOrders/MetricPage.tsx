...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/workorders/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'WorkOrders-Metric-' + (index + 1),
...
export default function WorkOrdersMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WorkOrders', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WorkOrders', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WorkOrders-Metric">
      <header>
        <h1>WorkOrders / Metric</h1>
