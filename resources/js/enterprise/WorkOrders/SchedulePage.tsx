...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/workorders/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'WorkOrders-Schedule-' + (index + 1),
...
export default function WorkOrdersSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['WorkOrders', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['WorkOrders', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module WorkOrders-Schedule">
      <header>
        <h1>WorkOrders / Schedule</h1>
