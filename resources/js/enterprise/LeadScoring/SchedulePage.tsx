...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/leadscoring/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'LeadScoring-Schedule-' + (index + 1),
...
export default function LeadScoringSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['LeadScoring', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['LeadScoring', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module LeadScoring-Schedule">
      <header>
        <h1>LeadScoring / Schedule</h1>
