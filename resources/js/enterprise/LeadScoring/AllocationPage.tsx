...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/leadscoring/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'LeadScoring-Allocation-' + (index + 1),
...
export default function LeadScoringAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['LeadScoring', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['LeadScoring', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module LeadScoring-Allocation">
      <header>
        <h1>LeadScoring / Allocation</h1>
