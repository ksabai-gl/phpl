...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/leadscoring/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'LeadScoring-Checkpoint-' + (index + 1),
...
export default function LeadScoringCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['LeadScoring', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['LeadScoring', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module LeadScoring-Checkpoint">
      <header>
        <h1>LeadScoring / Checkpoint</h1>
