...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/leadscoring/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'LeadScoring-Workflow-' + (index + 1),
...
export default function LeadScoringWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['LeadScoring', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['LeadScoring', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module LeadScoring-Workflow">
      <header>
        <h1>LeadScoring / Workflow</h1>
