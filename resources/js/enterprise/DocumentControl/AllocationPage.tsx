...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/documentcontrol/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'DocumentControl-Allocation-' + (index + 1),
...
export default function DocumentControlAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DocumentControl', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DocumentControl', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DocumentControl-Allocation">
      <header>
        <h1>DocumentControl / Allocation</h1>
