...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/documentcontrol/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'DocumentControl-Metric-' + (index + 1),
...
export default function DocumentControlMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DocumentControl', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DocumentControl', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DocumentControl-Metric">
      <header>
        <h1>DocumentControl / Metric</h1>
