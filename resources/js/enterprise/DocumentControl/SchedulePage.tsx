...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/documentcontrol/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'DocumentControl-Schedule-' + (index + 1),
...
export default function DocumentControlSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DocumentControl', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DocumentControl', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DocumentControl-Schedule">
      <header>
        <h1>DocumentControl / Schedule</h1>
