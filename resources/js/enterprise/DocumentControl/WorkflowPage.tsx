...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/documentcontrol/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'DocumentControl-Workflow-' + (index + 1),
...
export default function DocumentControlWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DocumentControl', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DocumentControl', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DocumentControl-Workflow">
      <header>
        <h1>DocumentControl / Workflow</h1>
