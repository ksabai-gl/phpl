...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/documentcontrol/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'DocumentControl-Record-' + (index + 1),
...
export default function DocumentControlRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['DocumentControl', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['DocumentControl', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module DocumentControl-Record">
      <header>
        <h1>DocumentControl / Record</h1>
