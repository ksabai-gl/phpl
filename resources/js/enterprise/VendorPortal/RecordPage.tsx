...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/vendorportal/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'VendorPortal-Record-' + (index + 1),
...
export default function VendorPortalRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['VendorPortal', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['VendorPortal', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module VendorPortal-Record">
      <header>
        <h1>VendorPortal / Record</h1>
