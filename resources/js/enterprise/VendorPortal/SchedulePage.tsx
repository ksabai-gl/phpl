...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/vendorportal/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'VendorPortal-Schedule-' + (index + 1),
...
export default function VendorPortalSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['VendorPortal', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['VendorPortal', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module VendorPortal-Schedule">
      <header>
        <h1>VendorPortal / Schedule</h1>
