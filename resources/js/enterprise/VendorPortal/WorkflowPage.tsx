...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/vendorportal/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'VendorPortal-Workflow-' + (index + 1),
...
export default function VendorPortalWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['VendorPortal', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['VendorPortal', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module VendorPortal-Workflow">
      <header>
        <h1>VendorPortal / Workflow</h1>
