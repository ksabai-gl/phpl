...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/vendorportal/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'VendorPortal-Checkpoint-' + (index + 1),
...
export default function VendorPortalCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['VendorPortal', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['VendorPortal', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module VendorPortal-Checkpoint">
      <header>
        <h1>VendorPortal / Checkpoint</h1>
