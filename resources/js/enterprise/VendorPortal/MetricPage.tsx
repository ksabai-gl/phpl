...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/vendorportal/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'VendorPortal-Metric-' + (index + 1),
...
export default function VendorPortalMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['VendorPortal', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['VendorPortal', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module VendorPortal-Metric">
      <header>
        <h1>VendorPortal / Metric</h1>
