...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/subscriptionsplus/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'SubscriptionsPlus-Schedule-' + (index + 1),
...
export default function SubscriptionsPlusSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SubscriptionsPlus', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SubscriptionsPlus', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SubscriptionsPlus-Schedule">
      <header>
        <h1>SubscriptionsPlus / Schedule</h1>
