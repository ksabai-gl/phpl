...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/subscriptionsplus/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'SubscriptionsPlus-Record-' + (index + 1),
...
export default function SubscriptionsPlusRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SubscriptionsPlus', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SubscriptionsPlus', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SubscriptionsPlus-Record">
      <header>
        <h1>SubscriptionsPlus / Record</h1>
