...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/subscriptionsplus/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'SubscriptionsPlus-Allocation-' + (index + 1),
...
export default function SubscriptionsPlusAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SubscriptionsPlus', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SubscriptionsPlus', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SubscriptionsPlus-Allocation">
      <header>
        <h1>SubscriptionsPlus / Allocation</h1>
