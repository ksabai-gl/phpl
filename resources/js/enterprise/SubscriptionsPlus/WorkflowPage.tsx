...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/subscriptionsplus/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'SubscriptionsPlus-Workflow-' + (index + 1),
...
export default function SubscriptionsPlusWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SubscriptionsPlus', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SubscriptionsPlus', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SubscriptionsPlus-Workflow">
      <header>
        <h1>SubscriptionsPlus / Workflow</h1>
