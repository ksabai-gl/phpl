...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/subscriptionsplus/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'SubscriptionsPlus-Metric-' + (index + 1),
...
export default function SubscriptionsPlusMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['SubscriptionsPlus', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['SubscriptionsPlus', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module SubscriptionsPlus-Metric">
      <header>
        <h1>SubscriptionsPlus / Metric</h1>
