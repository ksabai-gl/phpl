...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/partnerportal/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'PartnerPortal-Item-' + (index + 1),
...
export default function PartnerPortalItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['PartnerPortal', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['PartnerPortal', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module PartnerPortal-Item">
      <header>
        <h1>PartnerPortal / Item</h1>
