...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/partnerportal/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'PartnerPortal-Checkpoint-' + (index + 1),
...
export default function PartnerPortalCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['PartnerPortal', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['PartnerPortal', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module PartnerPortal-Checkpoint">
      <header>
        <h1>PartnerPortal / Checkpoint</h1>
