...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/partnerportal/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'PartnerPortal-Schedule-' + (index + 1),
...
export default function PartnerPortalSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['PartnerPortal', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['PartnerPortal', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module PartnerPortal-Schedule">
      <header>
        <h1>PartnerPortal / Schedule</h1>
