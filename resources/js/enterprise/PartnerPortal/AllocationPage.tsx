...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/partnerportal/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'PartnerPortal-Allocation-' + (index + 1),
...
export default function PartnerPortalAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['PartnerPortal', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['PartnerPortal', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module PartnerPortal-Allocation">
      <header>
        <h1>PartnerPortal / Allocation</h1>
