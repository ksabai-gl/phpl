...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/partnerportal/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'PartnerPortal-Metric-' + (index + 1),
...
export default function PartnerPortalMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['PartnerPortal', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['PartnerPortal', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module PartnerPortal-Metric">
      <header>
        <h1>PartnerPortal / Metric</h1>
