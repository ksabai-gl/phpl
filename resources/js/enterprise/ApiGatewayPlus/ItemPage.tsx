...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/apigatewayplus/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'ApiGatewayPlus-Item-' + (index + 1),
...
export default function ApiGatewayPlusItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ApiGatewayPlus', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ApiGatewayPlus', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ApiGatewayPlus-Item">
      <header>
        <h1>ApiGatewayPlus / Item</h1>
