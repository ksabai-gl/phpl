...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/apigatewayplus/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'ApiGatewayPlus-Workflow-' + (index + 1),
...
export default function ApiGatewayPlusWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ApiGatewayPlus', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ApiGatewayPlus', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ApiGatewayPlus-Workflow">
      <header>
        <h1>ApiGatewayPlus / Workflow</h1>
