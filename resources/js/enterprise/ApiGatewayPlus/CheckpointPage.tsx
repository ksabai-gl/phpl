...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/apigatewayplus/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'ApiGatewayPlus-Checkpoint-' + (index + 1),
...
export default function ApiGatewayPlusCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ApiGatewayPlus', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ApiGatewayPlus', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ApiGatewayPlus-Checkpoint">
      <header>
        <h1>ApiGatewayPlus / Checkpoint</h1>
