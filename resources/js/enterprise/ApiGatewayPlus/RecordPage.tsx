...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/apigatewayplus/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'ApiGatewayPlus-Record-' + (index + 1),
...
export default function ApiGatewayPlusRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ApiGatewayPlus', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ApiGatewayPlus', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ApiGatewayPlus-Record">
      <header>
        <h1>ApiGatewayPlus / Record</h1>
