...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/apigatewayplus/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'ApiGatewayPlus-Metric-' + (index + 1),
...
export default function ApiGatewayPlusMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ApiGatewayPlus', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ApiGatewayPlus', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ApiGatewayPlus-Metric">
      <header>
        <h1>ApiGatewayPlus / Metric</h1>
