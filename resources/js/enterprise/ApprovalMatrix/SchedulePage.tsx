...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/approvalmatrix/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'ApprovalMatrix-Schedule-' + (index + 1),
...
export default function ApprovalMatrixSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ApprovalMatrix', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ApprovalMatrix', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ApprovalMatrix-Schedule">
      <header>
        <h1>ApprovalMatrix / Schedule</h1>
