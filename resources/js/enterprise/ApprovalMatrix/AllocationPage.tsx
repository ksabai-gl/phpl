...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/approvalmatrix/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'ApprovalMatrix-Allocation-' + (index + 1),
...
export default function ApprovalMatrixAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ApprovalMatrix', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ApprovalMatrix', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ApprovalMatrix-Allocation">
      <header>
        <h1>ApprovalMatrix / Allocation</h1>
