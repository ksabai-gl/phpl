...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/approvalmatrix/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'ApprovalMatrix-Item-' + (index + 1),
...
export default function ApprovalMatrixItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ApprovalMatrix', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ApprovalMatrix', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ApprovalMatrix-Item">
      <header>
        <h1>ApprovalMatrix / Item</h1>
