...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/approvalmatrix/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'ApprovalMatrix-Record-' + (index + 1),
...
export default function ApprovalMatrixRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ApprovalMatrix', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ApprovalMatrix', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ApprovalMatrix-Record">
      <header>
        <h1>ApprovalMatrix / Record</h1>
