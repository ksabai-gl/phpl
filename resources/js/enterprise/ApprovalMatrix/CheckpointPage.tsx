...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/approvalmatrix/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'ApprovalMatrix-Checkpoint-' + (index + 1),
...
export default function ApprovalMatrixCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ApprovalMatrix', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ApprovalMatrix', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ApprovalMatrix-Checkpoint">
      <header>
        <h1>ApprovalMatrix / Checkpoint</h1>
