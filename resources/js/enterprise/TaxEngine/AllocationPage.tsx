...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/taxengine/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'TaxEngine-Allocation-' + (index + 1),
...
export default function TaxEngineAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['TaxEngine', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['TaxEngine', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module TaxEngine-Allocation">
      <header>
        <h1>TaxEngine / Allocation</h1>
