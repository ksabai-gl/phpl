...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/taxengine/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'TaxEngine-Checkpoint-' + (index + 1),
...
export default function TaxEngineCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['TaxEngine', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['TaxEngine', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module TaxEngine-Checkpoint">
      <header>
        <h1>TaxEngine / Checkpoint</h1>
