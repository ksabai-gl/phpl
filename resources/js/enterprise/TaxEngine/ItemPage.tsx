...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/taxengine/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'TaxEngine-Item-' + (index + 1),
...
export default function TaxEngineItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['TaxEngine', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['TaxEngine', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module TaxEngine-Item">
      <header>
        <h1>TaxEngine / Item</h1>
