...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/featureflags/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'FeatureFlags-Record-' + (index + 1),
...
export default function FeatureFlagsRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FeatureFlags', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FeatureFlags', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FeatureFlags-Record">
      <header>
        <h1>FeatureFlags / Record</h1>
