...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/featureflags/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'FeatureFlags-Allocation-' + (index + 1),
...
export default function FeatureFlagsAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FeatureFlags', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FeatureFlags', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FeatureFlags-Allocation">
      <header>
        <h1>FeatureFlags / Allocation</h1>
