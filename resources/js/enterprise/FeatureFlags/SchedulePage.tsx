...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/featureflags/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'FeatureFlags-Schedule-' + (index + 1),
...
export default function FeatureFlagsSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FeatureFlags', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FeatureFlags', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FeatureFlags-Schedule">
      <header>
        <h1>FeatureFlags / Schedule</h1>
