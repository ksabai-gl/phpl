...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/featureflags/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'FeatureFlags-Checkpoint-' + (index + 1),
...
export default function FeatureFlagsCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FeatureFlags', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FeatureFlags', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FeatureFlags-Checkpoint">
      <header>
        <h1>FeatureFlags / Checkpoint</h1>
