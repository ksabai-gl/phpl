...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/featureflags/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'FeatureFlags-Item-' + (index + 1),
...
export default function FeatureFlagsItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['FeatureFlags', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['FeatureFlags', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module FeatureFlags-Item">
      <header>
        <h1>FeatureFlags / Item</h1>
