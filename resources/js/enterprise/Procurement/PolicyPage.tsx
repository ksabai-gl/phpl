...
async function fetchPolicyRows(): Promise<PolicyRow[]> {
  const response = await fetch('/api/v1/enterprise/procurement/policy');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Policy ' + (index + 1),
      code: 'Procurement-Policy-' + (index + 1),
...
export default function ProcurementPolicyPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['Procurement', 'Policy', filter, status],
  ...
  return (
    <section className="enterprise-module Procurement-Policy">
      <header>
        <h1>Procurement / Policy</h1>
