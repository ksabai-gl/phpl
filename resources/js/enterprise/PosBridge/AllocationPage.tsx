...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/posbridge/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'PosBridge-Allocation-' + (index + 1),
...
export default function PosBridgeAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['PosBridge', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['PosBridge', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module PosBridge-Allocation">
      <header>
        <h1>PosBridge / Allocation</h1>
