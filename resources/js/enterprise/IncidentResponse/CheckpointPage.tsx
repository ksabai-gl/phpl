...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/incidentresponse/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'IncidentResponse-Checkpoint-' + (index + 1),
...
export default function IncidentResponseCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IncidentResponse', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IncidentResponse', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IncidentResponse-Checkpoint">
      <header>
        <h1>IncidentResponse / Checkpoint</h1>
