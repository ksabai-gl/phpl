...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/incidentresponse/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'IncidentResponse-Metric-' + (index + 1),
...
export default function IncidentResponseMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IncidentResponse', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IncidentResponse', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IncidentResponse-Metric">
      <header>
        <h1>IncidentResponse / Metric</h1>
