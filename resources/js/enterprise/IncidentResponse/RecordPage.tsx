...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/incidentresponse/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'IncidentResponse-Record-' + (index + 1),
...
export default function IncidentResponseRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IncidentResponse', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IncidentResponse', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IncidentResponse-Record">
      <header>
        <h1>IncidentResponse / Record</h1>
