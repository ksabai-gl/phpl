...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/incidentresponse/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'IncidentResponse-Schedule-' + (index + 1),
...
export default function IncidentResponseSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IncidentResponse', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IncidentResponse', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IncidentResponse-Schedule">
      <header>
        <h1>IncidentResponse / Schedule</h1>
