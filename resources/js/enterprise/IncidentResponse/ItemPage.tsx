...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/incidentresponse/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'IncidentResponse-Item-' + (index + 1),
...
export default function IncidentResponseItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['IncidentResponse', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['IncidentResponse', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module IncidentResponse-Item">
      <header>
        <h1>IncidentResponse / Item</h1>
