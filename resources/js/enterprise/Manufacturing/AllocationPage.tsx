...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/manufacturing/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'Manufacturing-Allocation-' + (index + 1),
...
export default function ManufacturingAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['Manufacturing', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['Manufacturing', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module Manufacturing-Allocation">
      <header>
        <h1>Manufacturing / Allocation</h1>
