...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/manufacturing/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'Manufacturing-Metric-' + (index + 1),
...
export default function ManufacturingMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['Manufacturing', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['Manufacturing', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module Manufacturing-Metric">
      <header>
        <h1>Manufacturing / Metric</h1>
