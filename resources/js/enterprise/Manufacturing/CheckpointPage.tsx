...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/manufacturing/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'Manufacturing-Checkpoint-' + (index + 1),
...
export default function ManufacturingCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['Manufacturing', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['Manufacturing', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module Manufacturing-Checkpoint">
      <header>
        <h1>Manufacturing / Checkpoint</h1>
