...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/ecommercebridge/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'EcommerceBridge-Checkpoint-' + (index + 1),
...
export default function EcommerceBridgeCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['EcommerceBridge', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['EcommerceBridge', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module EcommerceBridge-Checkpoint">
      <header>
        <h1>EcommerceBridge / Checkpoint</h1>
