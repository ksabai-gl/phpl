...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/ecommercebridge/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'EcommerceBridge-Schedule-' + (index + 1),
...
export default function EcommerceBridgeSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['EcommerceBridge', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['EcommerceBridge', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module EcommerceBridge-Schedule">
      <header>
        <h1>EcommerceBridge / Schedule</h1>
