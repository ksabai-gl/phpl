...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/ecommercebridge/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'EcommerceBridge-Record-' + (index + 1),
...
export default function EcommerceBridgeRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['EcommerceBridge', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['EcommerceBridge', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module EcommerceBridge-Record">
      <header>
        <h1>EcommerceBridge / Record</h1>
