...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/ecommercebridge/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'EcommerceBridge-Metric-' + (index + 1),
...
export default function EcommerceBridgeMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['EcommerceBridge', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['EcommerceBridge', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module EcommerceBridge-Metric">
      <header>
        <h1>EcommerceBridge / Metric</h1>
