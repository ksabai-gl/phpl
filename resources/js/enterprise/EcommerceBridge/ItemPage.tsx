...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/ecommercebridge/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'EcommerceBridge-Item-' + (index + 1),
...
export default function EcommerceBridgeItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['EcommerceBridge', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['EcommerceBridge', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module EcommerceBridge-Item">
      <header>
        <h1>EcommerceBridge / Item</h1>
