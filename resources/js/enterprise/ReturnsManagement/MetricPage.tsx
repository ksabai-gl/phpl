...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/returnsmanagement/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'ReturnsManagement-Metric-' + (index + 1),
...
export default function ReturnsManagementMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ReturnsManagement', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ReturnsManagement', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ReturnsManagement-Metric">
      <header>
        <h1>ReturnsManagement / Metric</h1>
