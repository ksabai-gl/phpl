...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/returnsmanagement/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'ReturnsManagement-Record-' + (index + 1),
...
export default function ReturnsManagementRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ReturnsManagement', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ReturnsManagement', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ReturnsManagement-Record">
      <header>
        <h1>ReturnsManagement / Record</h1>
