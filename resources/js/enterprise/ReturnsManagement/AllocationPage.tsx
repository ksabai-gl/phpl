...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/returnsmanagement/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'ReturnsManagement-Allocation-' + (index + 1),
...
export default function ReturnsManagementAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ReturnsManagement', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ReturnsManagement', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ReturnsManagement-Allocation">
      <header>
        <h1>ReturnsManagement / Allocation</h1>
