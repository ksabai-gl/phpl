...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/returnsmanagement/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'ReturnsManagement-Checkpoint-' + (index + 1),
...
export default function ReturnsManagementCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ReturnsManagement', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ReturnsManagement', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ReturnsManagement-Checkpoint">
      <header>
        <h1>ReturnsManagement / Checkpoint</h1>
