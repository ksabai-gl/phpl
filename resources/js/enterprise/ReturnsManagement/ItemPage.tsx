...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/returnsmanagement/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'ReturnsManagement-Item-' + (index + 1),
...
export default function ReturnsManagementItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ReturnsManagement', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ReturnsManagement', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ReturnsManagement-Item">
      <header>
        <h1>ReturnsManagement / Item</h1>
