...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/qualitycontrol/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'QualityControl-Schedule-' + (index + 1),
...
export default function QualityControlSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['QualityControl', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['QualityControl', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module QualityControl-Schedule">
      <header>
        <h1>QualityControl / Schedule</h1>
