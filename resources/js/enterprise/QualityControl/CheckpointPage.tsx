...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/qualitycontrol/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'QualityControl-Checkpoint-' + (index + 1),
...
export default function QualityControlCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['QualityControl', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['QualityControl', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module QualityControl-Checkpoint">
      <header>
        <h1>QualityControl / Checkpoint</h1>
