...
async function fetchScheduleRows(): Promise<ScheduleRow[]> {
  const response = await fetch('/api/v1/enterprise/performancereviews/schedule');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Schedule ' + (index + 1),
      code: 'PerformanceReviews-Schedule-' + (index + 1),
...
export default function PerformanceReviewsSchedulePage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['PerformanceReviews', 'Schedule', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['PerformanceReviews', 'Schedule'] });
    },
  });
  ...
  return (
    <section className="enterprise-module PerformanceReviews-Schedule">
      <header>
        <h1>PerformanceReviews / Schedule</h1>
