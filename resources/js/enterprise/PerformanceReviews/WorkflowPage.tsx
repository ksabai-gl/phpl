...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/performancereviews/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'PerformanceReviews-Workflow-' + (index + 1),
...
export default function PerformanceReviewsWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['PerformanceReviews', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['PerformanceReviews', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module PerformanceReviews-Workflow">
      <header>
        <h1>PerformanceReviews / Workflow</h1>
