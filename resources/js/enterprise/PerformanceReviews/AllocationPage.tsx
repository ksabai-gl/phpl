...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/performancereviews/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'PerformanceReviews-Allocation-' + (index + 1),
...
export default function PerformanceReviewsAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['PerformanceReviews', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['PerformanceReviews', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module PerformanceReviews-Allocation">
      <header>
        <h1>PerformanceReviews / Allocation</h1>
