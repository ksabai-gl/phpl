...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/performancereviews/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'PerformanceReviews-Record-' + (index + 1),
...
export default function PerformanceReviewsRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['PerformanceReviews', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['PerformanceReviews', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module PerformanceReviews-Record">
      <header>
        <h1>PerformanceReviews / Record</h1>
