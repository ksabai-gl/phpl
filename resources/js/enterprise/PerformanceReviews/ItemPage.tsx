...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/performancereviews/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'PerformanceReviews-Item-' + (index + 1),
...
export default function PerformanceReviewsItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['PerformanceReviews', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['PerformanceReviews', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module PerformanceReviews-Item">
      <header>
        <h1>PerformanceReviews / Item</h1>
