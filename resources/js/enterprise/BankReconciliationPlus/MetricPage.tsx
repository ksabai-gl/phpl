...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/bankreconciliationplus/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'BankReconciliationPlus-Metric-' + (index + 1),
...
export default function BankReconciliationPlusMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['BankReconciliationPlus', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['BankReconciliationPlus', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module BankReconciliationPlus-Metric">
      <header>
        <h1>BankReconciliationPlus / Metric</h1>
