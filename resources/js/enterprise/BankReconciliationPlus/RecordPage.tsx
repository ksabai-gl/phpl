...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/bankreconciliationplus/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'BankReconciliationPlus-Record-' + (index + 1),
...
export default function BankReconciliationPlusRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['BankReconciliationPlus', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['BankReconciliationPlus', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module BankReconciliationPlus-Record">
      <header>
        <h1>BankReconciliationPlus / Record</h1>
