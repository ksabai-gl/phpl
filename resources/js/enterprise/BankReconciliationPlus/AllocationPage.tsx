...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/bankreconciliationplus/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'BankReconciliationPlus-Allocation-' + (index + 1),
...
export default function BankReconciliationPlusAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['BankReconciliationPlus', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['BankReconciliationPlus', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module BankReconciliationPlus-Allocation">
      <header>
        <h1>BankReconciliationPlus / Allocation</h1>
