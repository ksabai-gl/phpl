...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/bankreconciliationplus/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'BankReconciliationPlus-Checkpoint-' + (index + 1),
...
export default function BankReconciliationPlusCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['BankReconciliationPlus', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['BankReconciliationPlus', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module BankReconciliationPlus-Checkpoint">
      <header>
        <h1>BankReconciliationPlus / Checkpoint</h1>
