...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/bankreconciliationplus/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'BankReconciliationPlus-Item-' + (index + 1),
...
export default function BankReconciliationPlusItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['BankReconciliationPlus', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['BankReconciliationPlus', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module BankReconciliationPlus-Item">
      <header>
        <h1>BankReconciliationPlus / Item</h1>
