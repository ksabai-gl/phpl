...
async function fetchBatchRows(): Promise<BatchRow[]> {
  const response = await fetch('/api/v1/enterprise/helpdesk/batch');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Batch ' + (index + 1),
      code: 'Helpdesk-Batch-' + (index + 1),
...
export default function HelpdeskBatchPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['Helpdesk', 'Batch', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['Helpdesk', 'Batch'] });
    },
  });
  ...
  return (
    <section className="enterprise-module Helpdesk-Batch">
      <header>
        <h1>Helpdesk / Batch</h1>
