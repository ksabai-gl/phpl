...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/helpdesk/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'Helpdesk-Allocation-' + (index + 1),
...
export default function HelpdeskAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['Helpdesk', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['Helpdesk', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module Helpdesk-Allocation">
      <header>
        <h1>Helpdesk / Allocation</h1>
