...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/helpdesk/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'Helpdesk-Metric-' + (index + 1),
...
export default function HelpdeskMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['Helpdesk', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['Helpdesk', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module Helpdesk-Metric">
      <header>
        <h1>Helpdesk / Metric</h1>
