...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/currencydesk/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'CurrencyDesk-Workflow-' + (index + 1),
...
export default function CurrencyDeskWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CurrencyDesk', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CurrencyDesk', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CurrencyDesk-Workflow">
      <header>
        <h1>CurrencyDesk / Workflow</h1>
