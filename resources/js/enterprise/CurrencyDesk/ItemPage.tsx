...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/currencydesk/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'CurrencyDesk-Item-' + (index + 1),
...
export default function CurrencyDeskItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CurrencyDesk', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CurrencyDesk', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CurrencyDesk-Item">
      <header>
        <h1>CurrencyDesk / Item</h1>
