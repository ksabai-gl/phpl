...
async function fetchRecordRows(): Promise<RecordRow[]> {
  const response = await fetch('/api/v1/enterprise/currencydesk/record');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Record ' + (index + 1),
      code: 'CurrencyDesk-Record-' + (index + 1),
...
export default function CurrencyDeskRecordPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CurrencyDesk', 'Record', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CurrencyDesk', 'Record'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CurrencyDesk-Record">
      <header>
        <h1>CurrencyDesk / Record</h1>
