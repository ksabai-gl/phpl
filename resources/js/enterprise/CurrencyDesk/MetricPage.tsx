...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/currencydesk/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'CurrencyDesk-Metric-' + (index + 1),
...
export default function CurrencyDeskMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CurrencyDesk', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CurrencyDesk', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CurrencyDesk-Metric">
      <header>
        <h1>CurrencyDesk / Metric</h1>
