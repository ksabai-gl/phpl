...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/currencydesk/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'CurrencyDesk-Checkpoint-' + (index + 1),
...
export default function CurrencyDeskCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['CurrencyDesk', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['CurrencyDesk', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module CurrencyDesk-Checkpoint">
      <header>
        <h1>CurrencyDesk / Checkpoint</h1>
