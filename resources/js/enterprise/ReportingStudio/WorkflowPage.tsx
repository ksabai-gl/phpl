...
async function fetchWorkflowRows(): Promise<WorkflowRow[]> {
  const response = await fetch('/api/v1/enterprise/reportingstudio/workflow');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Workflow ' + (index + 1),
      code: 'ReportingStudio-Workflow-' + (index + 1),
...
export default function ReportingStudioWorkflowPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ReportingStudio', 'Workflow', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ReportingStudio', 'Workflow'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ReportingStudio-Workflow">
      <header>
        <h1>ReportingStudio / Workflow</h1>
