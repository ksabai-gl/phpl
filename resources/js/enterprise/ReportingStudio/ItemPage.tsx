...
async function fetchItemRows(): Promise<ItemRow[]> {
  const response = await fetch('/api/v1/enterprise/reportingstudio/item');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Item ' + (index + 1),
      code: 'ReportingStudio-Item-' + (index + 1),
...
export default function ReportingStudioItemPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ReportingStudio', 'Item', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ReportingStudio', 'Item'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ReportingStudio-Item">
      <header>
        <h1>ReportingStudio / Item</h1>
