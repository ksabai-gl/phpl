...
async function fetchMetricRows(): Promise<MetricRow[]> {
  const response = await fetch('/api/v1/enterprise/reportingstudio/metric');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Metric ' + (index + 1),
      code: 'ReportingStudio-Metric-' + (index + 1),
...
export default function ReportingStudioMetricPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ReportingStudio', 'Metric', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ReportingStudio', 'Metric'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ReportingStudio-Metric">
      <header>
        <h1>ReportingStudio / Metric</h1>
