...
async function fetchCheckpointRows(): Promise<CheckpointRow[]> {
  const response = await fetch('/api/v1/enterprise/reportingstudio/checkpoint');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Checkpoint ' + (index + 1),
      code: 'ReportingStudio-Checkpoint-' + (index + 1),
...
export default function ReportingStudioCheckpointPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ReportingStudio', 'Checkpoint', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ReportingStudio', 'Checkpoint'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ReportingStudio-Checkpoint">
      <header>
        <h1>ReportingStudio / Checkpoint</h1>
