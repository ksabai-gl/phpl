...
async function fetchAllocationRows(): Promise<AllocationRow[]> {
  const response = await fetch('/api/v1/enterprise/reportingstudio/allocation');
  if (!response.ok) {
    return Array.from({ length: 12 }, (_, index) => ({
      id: index + 1,
      name: 'Allocation ' + (index + 1),
      code: 'ReportingStudio-Allocation-' + (index + 1),
...
export default function ReportingStudioAllocationPage(): React.JSX.Element {
  const queryClient = useQueryClient();
  const [filter, setFilter] = useState('');
  const [status, setStatus] = useState('all');

  const query = useQuery({
    queryKey: ['ReportingStudio', 'Allocation', filter, status],
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['ReportingStudio', 'Allocation'] });
    },
  });
  ...
  return (
    <section className="enterprise-module ReportingStudio-Allocation">
      <header>
        <h1>ReportingStudio / Allocation</h1>
